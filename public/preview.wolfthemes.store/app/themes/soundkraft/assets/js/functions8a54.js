 ...
 */
/* jshint -W062 */
/* global SoundkraftParams, WOW, AOS, WVC, WolfCore, Cookies, Event, objectFitImages, WVCBigText, WVCParams, SoundkraftYTVideoBg, Vimeo */

var SoundkraftUi = (function ($) {
    ...
    return {
         ...
         */
        init: function () {
            this.isMobile = SoundkraftParams.isMobile;
			this.mobileScreenBreakpoint =
				SoundkraftParams.mobileScreenBreakpoint;
			this.tabletScreenBreakpoint =
				SoundkraftParams.tabletScreenBreakpoint;
			this.notebookScreenBreakpoint =
				SoundkraftParams.notebookScreenBreakpoint;
			this.desktopScreenBreakpoint =
				SoundkraftParams.desktopScreenBreakpoint;
			this.desktopBigScreenBreakpoint =
				SoundkraftParams.desktopBigScreenBreakpoint;
		...
		setClasses: function () {
			...
			if (
				...
				!SoundkraftParams.forceAnimationMobile
			) {
		...
		loadingAnimation: function () {
			if (!SoundkraftParams.defaultPageLoadingAnimation) {
				return false;
			}
			...
			if ($.isFunction($.fn.stick_in_parent)) {
				if (
					...
					$(".work-info-container").stick_in_parent({
						offset_top: parseInt(
							SoundkraftParams.portfolioSidebarOffsetTop,
							10
						),
					});
				}
		...
		mobileMenuBreakPoint: function ( hard ) {
			...
			var hard = hard || false,
				...
				breakpoint = SoundkraftParams.breakPoint;
			...
			if (breakpoint > winWidth) {
				...
				$(window).trigger("soundkraft_breakpoint");
		...
		subMenuDirection: function () {
			var $this,
				subMenuWidth = parseInt(SoundkraftParams.subMenuWidth, 10),
				bleed = 8;
		...
		stickyMenu: function (scrollTop) {
			var scrollPoint,
				menuOffset = parseInt(SoundkraftParams.menuOffset, 10);

			scrollTop = scrollTop || 0;

			scrollPoint =
				parseInt(SoundkraftParams.stickyMenuScrollPoint, 10) ||
				$("body").offset().top;
			...
			if (menuOffset) {
				if ("100%" === SoundkraftParams.menuOffset) {
					scrollPoint =
						$(window).height() -
						parseInt(SoundkraftParams.desktopMenuHeight, 10);
				} else {
					scrollPoint =
						menuOffset -
						parseInt(SoundkraftParams.desktopMenuHeight, 10);
			...
			if ("soft" === SoundkraftParams.stickyMenuType) {
			...
			} else if ("hard" === SoundkraftParams.stickyMenuType) {
				if (scrollPoint < scrollTop) {
		...
		parallax: function () {
			var smallScreen =
				...
				SoundkraftParams.parallaxNoSmallScreen;

			if (!smallScreen && typeof jarallax === 'function') {
				$(".parallax").jarallax({
					noAndroid: SoundkraftParams.parallaxNoAndroid,
					noIos: SoundkraftParams.parallaxNoIos,
				});
			}
		},
		...
		toggleMenu: function () {
			...
			$(document).on("click", ".toggle-mobile-menu", function (event) {
				...
				$(window).trigger("soundkraft_mobile_menu_toggle_button_click", [
					$(this),
				]);
			...
			$(document).on("click", ".toggle-side-panel", function (event) {
				event.preventDefault();
				$(window).trigger("soundkraft_side_panel_toggle_button_click", [
					$(this),
				]);
			...
			$(document).on("click", ".toggle-offcanvas-menu", function (event) {
				event.preventDefault();
				$( window ).trigger("soundkraft_offcanvas_menu_toggle_button_click", [
					$(this),
				]);
			...
			$(document).on("click", ".toggle-overlay-menu", function (event) {
				...
				$(window).trigger(
					"soundkraft_overlay_menu_toggle_button_click",
					[$(this)]
				);
			...
			$(document).on("click", ".toggle-lateral-menu", function (event) {
				event.preventDefault();
				$(window).trigger(
					"soundkraft_lateral_menu_toggle_button_click",
					[$(this)]
				);
		...
		toggleSearchForm: function () {
			$(document).on('click', '.toggle-search', function ( event ) {
				event.preventDefault();
				$(window).trigger('soundkraft_searchform_toggle');
		...
		flexSlider: function () {
			if ($.isFunction($.flexslider)) {
				...
				$(".entry-slider").flexslider({
					animation: SoundkraftParams.entrySliderAnimation,
					slideshow: true,
					slideshowSpeed: 4000,
				});
			}
		...
		lightbox: function () {
			...
			if ("fancybox" === SoundkraftParams.lightbox) {
				$(selectors).fancybox(SoundkraftParams.fancyboxSettings);
				...
				$(".gallery-quickview, .wvc-gallery-quickview, .wolf-core-gallery-quickview")
					...
					.on("click", function ( event ) {
						...
						params = SoundkraftParams.fancyboxSettings;
				...
				$(".woocommerce-product-gallery__trigger")
					...
					.on("click", function ( event ) {
						...
						$.fancybox.open(
							quickviewData,
							SoundkraftParams.fancyboxSettings
						);
						return false;
					});
				...
				$(".lightbox-video, .wp-block-image.video-box a, .rotating-text-video-lightbox a").fancybox({
					beforeLoad: function () {
						SoundkraftUi.pausePlayers();
					},

					afterLoad: function () {
						SoundkraftUi.lightboxVideoAfterLoad();
					},

					afterClose: function () {
						SoundkraftUi.restartVideoBackgrounds();
					},
				});
			}
		},
		...
		lightboxVideoAfterLoad: function () {
			var $iframe = $(".fancybox-iframe").contents(),
				...
				accentColor = SoundkraftParams.accentColor;
			...
			if ($video.length) {
				...
				$head.append(
					$("<link/>", {
						rel: "stylesheet",
						href: SoundkraftParams.mediaelementLegacyCssUri,
						type: "text/css",
					})
				);
				$head.append(
					$("<link/>", {
						rel: "stylesheet",
						href: SoundkraftParams.mediaelementCssUri,
						type: "text/css",
					})
				);
				...
				$head.append(
					$("<link/>", {
						rel: "stylesheet",
						href: SoundkraftParams.fancyboxMediaelementCssUri,
						type: "text/css",
					})
				);
		...
		scrollToMainContent: function () {
			...
			$("html, body")
				...
				.animate(
					...
					parseInt(SoundkraftParams.smoothScrollSpeed, 10),
					SoundkraftParams.smoothScrollEase,
					function () {
						if ( "" !== hash) {
		...
		smoothScroll: function (href) {
			...
			var scrollOffset = this.getToolBarOffset() - 5 - parseInt( SoundkraftParams.smoothScrollOffsetAdjust ),
				hash;
			...
			if (-1 !== href.indexOf("#")) {
				...
				if ($("#" + hash).length) {
					if (
						"hard" === SoundkraftParams.stickyMenuType &&
						...
						scrollOffset += parseFloat(
							SoundkraftParams.stickyMenuHeight,
							10
						);
					}
		...
		wowAnimate: function () {
			...
			if ("undefined" !== typeof WOW) {
				wowAnimate = new WOW({
					boxClass: SoundkraftParams.WOWBoxClass,
					offset: SoundkraftParams.WOWAnimationOffset,
		 ...
		 */
		liveSearch: function () {
			if (!SoundkraftParams.doLiveSearch) {
				return;
			}
			...
			$(".live-search-form").each(function () {
				var $formContainer = $(this),
					...
					timer = null,
					$resultContainer,
					action = "soundkraft_ajax_live_search",
					result;

				if ($form.hasClass("woocommerce-product-search")) {
					action = "soundkraft_ajax_woocommerce_live_search";
				}
				...
				searchInput.on("keyup", function (event) {
					...
					if (8 === event.keyCode || 46 === event.keyCode) {
						...
						timer = setTimeout(function () {
							...
							$.post(SoundkraftParams.ajaxUrl, data, function (
								response
							) {
								if ("" !== response) {
			...
			if (!this.isMobile) {
				...
				$(document).on("added_to_cart", function (
					...
					if (
						...
						$tipspan.attr(
							"title",
							SoundkraftParams.l10n.addedToCart
						);
						...
						setTimeout(function () {
							...
							$tipspan.attr(
								"title",
								SoundkraftParams.l10n.addToCart
							);
							$tipspan.tooltipsy();
		...
		setInternalLinkClass: function () {
			var siteURL = SoundkraftParams.siteUrl,
				$internalLinks,
				regEx = "";

			$.each(SoundkraftParams.allowedMimeTypes, function (
				index,
				value
			) {
				regEx += "|" + value;
			...
			siteURL = SoundkraftParams.siteUrl;
			...
			if (SoundkraftParams.isWooCommerce) {
				/*
				...
				if (
					...
					this.untrailingSlashit(
						SoundkraftParams.WooCommerceCartUrl
					)
				) {
					$(
						"a[href^='" +
				...
				if (
					this.untrailingSlashit(siteURL) !==
					this.untrailingSlashit(
						SoundkraftParams.WooCommerceAccountUrl
					)
				) {
					$(
						"a[href^='" +
				...
				if (
					this.untrailingSlashit(siteURL) !==
					this.untrailingSlashit(
						SoundkraftParams.WooCommerceCheckoutUrl
					)
				) {
					$(
						"a[href^='" +
		...
		transitionCosmetic: function () {
			if (!SoundkraftParams.defaultPageTransitionAnimation) {
				return false;
			}

			var _this = this;

			if (SoundkraftParams.isAjaxNav) {
				return;
			}
			...
			$(document).on("click", ".internal-link:not(.disabled)", function (
				...
				if (!event.ctrlKey) {
					...
					if (SoundkraftParams.hasLoadingOverlay) {
						$(".loading-overlay").one(
		...
		videoThumbnailPlayOnHover: function () {
			...
			$("iframe.youtube-bg", itemsContainer).each(function () {
				if (
					"undefined" !== typeof SoundkraftYTVideoBg &&
					SoundkraftYTVideoBg.players[
				...
				if ($iframe.length) {
					if ($iframe.hasClass("youtube-bg")) {
						$(this).on("mouseenter", function () {
							if (
								"undefined" !== typeof SoundkraftYTVideoBg &&
								SoundkraftYTVideoBg.players[YTPlayerId]
							) {
								SoundkraftYTVideoBg.players[
									YTPlayerId
								].playVideo();
			...
			if ($("body").hasClass("single-artist")) {
				$("#artist-tabs").tabs({
					...
					activate: function (event) {
						...
						if ( SoundkraftParams.isWolfCore ) {
		...
		navPlayer: function () {
			$(document).on("click", ".nav-play-button", function () {
				...
				var $btn = $(this),
					...
					audioId = $audio.attr("id"),
					audio = document.getElementById(audioId),
					playText = SoundkraftParams.l10n.playText,
					pauseText = SoundkraftParams.l10n.pauseText;
		...
		revSliderErrorMessage : function() {
			if ( SoundkraftParams.isWolfCore && $( '.rs_error_message_box' ).length ) {
				$( '.rs_error_message_content' ).after( '<div class="rs_error_message_content">' + SoundkraftParams.l10n.missingRevsliderHelpMessage + '</div>' );
			}
		},
		...
		pageLoad: function () {
			...
			delay = SoundkraftParams.pageTransitionDelayAfter;
			...
			}, parseInt(SoundkraftParams.pageLoadedDelay, 10));
		},
	};
})(jQuery);
...
(function ($) {
	...
	if ("undefined" !== typeof WVC) {
		 ...
		 */
		WVC.lightbox = SoundkraftUi.lightbox;

		/**
		 * Overwrite WVC video opener
		 */
		WVC.videoOpener = SoundkraftUi.videoOpener;

		/**
		 * Overwrite toolbar offset calculation
		 */
		WVC.getToolBarOffset = SoundkraftUi.getToolBarOffset;
	}
	...
	if ("undefined" !== typeof WolfCore) {
		 ...
		 * Overwrite WVC lightbox function with the theme function
		 */
		WolfCore.lightbox = SoundkraftUi.lightbox;

		/**
		 * Overwrite WolfCore video opener
		 */
		WolfCore.videoOpener = SoundkraftUi.videoOpener;

		/**
		 * Overwrite toolbar offset calculation
		 */
		WolfCore.getToolBarOffset = SoundkraftUi.getToolBarOffset;
	}
	...
	$(window).on("load", function() {
		SoundkraftUi.pageLoad();

		setTimeout( function() {
			SoundkraftUi.fullyLoaded = true;
		}, 4000 );
